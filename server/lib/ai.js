/* ===================================================================
   lib/ai.js — primary model plus conditional second opinion.
   The fallback is used only when Beast genuinely signals uncertainty
   or the primary provider fails. Secrets stay server-side.
   =================================================================== */
...
export function shouldFallback(text) {
  ...
  return !value || value.includes(OUT_OF_KNOWLEDGE) || UNCERTAIN.test(value);
}
...
async function openAiFallback({ prompt, system }) {
  // No fallback call unless every fallback setting is explicitly configured.
...
export async function aiGenerate({ prompt, system }) {
  ...
  try {
    ...
    if (!shouldFallback(reply)) return reply;
    ...
    try {
      ...
      // Keep the primary error as the public failure category; never expose
      // provider payloads, credentials, or internal request details.
