"""Small, provider-agnostic model router for Beast.

Beast remains an astronomy/space specialist because that is the repository's
actual product role. The router only changes model selection and fallback.
"""
import json
import os
...
def should_fallback(text):
    ...
    if not value or UNCERTAINTY_MARKER.lower() in value:
        return True
    phrases = (
...
def call(base_url, api_key, model, messages, temperature=0.35):
...
def fallback(system, prompt, history):
    ...
    base = os.environ.get("OPENAI_API_URL", "https://api.openai.com/v1").strip()
    ...
    return call(base, key, model, messages)
